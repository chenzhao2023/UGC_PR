"""
UGCP network based on a 2D U-Net backbone.
...
class Unet2D_UGCP(nn.Module):
    """
    2D U-Net with UGCP refinement for binary segmentation.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        print("Unet2D_UGCP initialized. UGCP:", self.use_ugcp)

    def forward(self, x):
        backbone_features = self.backbone(x)  # [B, C, H, W]
        aligned_feat = self.feat_head(backbone_features)  # [B, 2, H, W]
        logits = self.logit_head(backbone_features)  # [B, 2, H, W]
